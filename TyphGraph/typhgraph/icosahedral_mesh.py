...
def subdivide_face_level(mesh: TriangularMesh, faces: np.ndarray) -> TriangularMesh:
    """
    mesh: TriangularMesh 객체
    faces: [M,3] array. subdivide 하고 싶은 원래 mesh.faces의 서브셋.
    반환: faces에 해당하는 삼각형만 한 단계 분할한 새로운 TriangularMesh
    """
    # Child vertex 관리를 위한 builder
    builder = _ChildVerticesBuilder(mesh.vertices)
    faces_to_refine = {tuple(face) for face in faces}

    new_faces = []
    # 원본 face 하나씩 순회
    for ind1, ind2, ind3 in mesh.faces:
        if (ind1, ind2, ind3) in faces_to_refine:
            # 4개로 쪼개기
    ...
    # builder에 저장된 모든 정점과 새로 만든 face 목록으로 mesh 구성
    return TriangularMesh(
        vertices=builder.get_all_vertices(),
        faces=np.array(new_faces, dtype=np.int32))


def get_regionally_refined_mesh(base_splits: int,
                                region_splits: int,
    ...
    """
    # 1) 균일 메쉬 생성
    meshes = get_hierarchy_of_triangular_meshes_for_sphere(splits=base_splits)
    finest = meshes[-1]

    lat_min, lat_max, lon_min, lon_max = region_bbox

    # 2) 추가 subdivide
    for _ in range(region_splits - base_splits):
        # 각 face(삼각형)의 중심을 구해서 bbox 체크
        centers = np.mean(finest.vertices[finest.faces], axis=1)  # Cartesian
        phi, theta = model_utils.cartesian_to_spherical(
            centers[:, 0], centers[:, 1], centers[:, 2])
        lats, lons = model_utils.spherical_to_lat_lon(phi, theta)

        # bbox 안에 있는 face 인덱스만 골라 subdivide
        mask = ((lats >= lat_min) & (lats <= lat_max) &
                (lons >= lon_min) & (lons <= lon_max))
        faces_to_refine = finest.faces[mask]

        # subdivide_face_level을 face 단위로 호출할 수 있다면,
        # 아니면 전체 subdivide 후 coarse한 삼각형 복원 로직이 필요
        finest = subdivide_face_level(finest, faces=faces_to_refine)
        meshes.append(finest)
    return meshes
